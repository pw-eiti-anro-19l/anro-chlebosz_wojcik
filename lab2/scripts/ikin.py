...
def inverted_kinematics(data):
    ...
    # rospy.logerr(a1)
    # rospy.logerr(dz)
    # rospy.logerr((-a2**2 + a1**2 + dz**2) / (2 * a1 * dz))
    try:
        alfa2 = acos((-a2**2 + a1**2 + dz**2) / (2 * a1 * dz))
        alfa3 = acos((a1**2 + a2**2 - dz**2) / (2 * a1 * a2))
    except:
        rospy.logerr('Too far for me, sorry :(')
        return
    ...
    # if (abs(current_pos[0] - theta1[0]) + abs(current_pos[1] - theta2[0]) < abs(current_pos[0] - theta1[1]) + abs(current_pos[1] - theta2[1])):
    th1 = theta1[0]
    th2 = theta2[0]
    # else:
    #     th1 = theta1[1]
...
if __name__ == "__main__":
    ...
    sub = rospy.Subscriber('ointerpolation', PoseStamped, inverted_kinematics)
